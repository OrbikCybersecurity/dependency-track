...
class ProjectReanalyzedNotificationEmitterTest extends PersistenceCapableTest {

    private ProjectReanalyzedNotificationEmitter emitter;

    @BeforeEach
    void beforeEach() {
        emitter = new ProjectReanalyzedNotificationEmitter();
        createCatchAllNotificationRule(qm, NotificationScope.PORTFOLIO);
    }

    @Test
    void shouldEmitProjectReanalyzedNotificationForCompletedRun() {
    ...
    @Test
    void shouldEmitProjectReanalyzedFailedNotificationForFailedRun() {
        final var project = new Project();
        project.setName("acme-app");
        qm.persist(project);
    ...
    void shouldEmitNotificationsForMultipleRuns() {
        ...
        emitter.onEvent(new WorkflowRunsCompletedEvent(List.of(
                createRunMetadata(
                        "vuln-analysis",
                        WorkflowRunStatus.COMPLETED,
                        Map.of(WF_LABEL_PROJECT_UUID, projectA.getUuid().toString())),
                createRunMetadata(
                        "vuln-analysis",
                        WorkflowRunStatus.FAILED,
    ...
    @Test
    void shouldTreatCancelledRunAsFailure() {
        final var project = new Project();
        project.setName("acme-app");
        qm.persist(project);
