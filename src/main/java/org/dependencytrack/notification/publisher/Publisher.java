        ...
        try (QueryManager qm = new QueryManager()) {
            ...
            if (NotificationScope.PORTFOLIO.name().equals(notification.getScope())) {
                if (notification.getSubject() instanceof final NewVulnerabilityIdentified subject) {
                    ...
                    //  to JSON "just in case" if it can be a rather large object graph...
                    context.put("subjectJson", NotificationUtil.toJson(subject));
                } else if (notification.getSubject() instanceof final ProjectReanalyzed subject) {
                    context.put("subject", subject);
                    context.put("subjectJson", NotificationUtil.toJson(subject));
                } else if (notification.getSubject() instanceof final ProjectReanalyzedFailed subject) {
                    context.put("subject", subject);
                    context.put("subjectJson", NotificationUtil.toJson(subject));
                }
